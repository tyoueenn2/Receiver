...
| Synthetic button and click checks | UPX1/UPC1/UPA1 v2/UPT3 framing, holds, zero-motion transitions, heartbeats, release fences, click ACK faults, metrics, and Pi epochs | Included executable |
...
For command-line validation, `receiver_test_hub_runner.exe <python.exe>` runs the same first eight checks and fails if any does not pass. The fake proxy never accesses USB. Automatic results do not establish real cursor delivery, Pi USB behavior, destination consumption, TensorRT detection quality, or deployment latency.
