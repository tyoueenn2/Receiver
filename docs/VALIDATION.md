...
- A terminal `Cancelled` ReleaseAll is replaced atomically by a strictly higher command ID in the same client session. The old obligation is marked superseded only as the replacement takes its place under the existing fence. Timeouts, packet loss, and nonterminal states continue retrying byte-identical data with the original ID. Repeated disarm/error triggers coalesce to one live release obligation.
- Delayed and duplicate replies for superseded releases cannot open the fence. Startup before endpoint readiness, repeated layout/disarm events, telemetry epoch changes, disconnect/reconnect behavior, and lost replies are covered by the native state-machine tests.
- `ButtonActive` with zero counts remains a pre-acceptance rejection. A full accepted count plus a smaller completed count is accepted as legitimate post-acceptance physical interference, recorded once as interrupted partial progress, and remains idempotent under duplicate replies. Partial acceptance and `completed >= accepted` for this terminal status remain protocol errors.
- Literal golden packets derived from the pinned proxy codec verify UPX1 (16 bytes), UPC1 v2 (40), UPA1 v2 (48), and UPT3 (128). The canonical UPT2 (80) decoder is checked independently. A representative packet from the pinned proxy's conflicting 80-byte UPT2 encoder is explicitly rejected. Runtime negotiation is UPT3 first, then UPT1; it never guesses between the two 80-byte UPT2 meanings.
- The real headless Receiver loopback checks one persistent UDP source endpoint across subscription, movement, click, retry, and release traffic. It also exercises a cached-cancelled release followed by a higher-ID replacement and a click that is accepted before physical `ButtonActive` interference.

Final local regression:

- The native Windows non-GPU build completed for the core library, headless runner, GUI, Test Hub, decoder, and all test executables.
- CTest passed 5/5 suites: core, humanization, tracking, injection, and pinned proxy codec vectors.
- Standalone executables reported 76,864 core checks, 239 synthetic-input/UPC1/UPA1 checks, and 6 pinned cross-project codec checks.
- The main network integration passed active output, physical release/resume, frame stall/recovery, telemetry expiry/recovery, old capture rejection, sender restart with a shape/format and clock change, Pi-not-ready handling, disarmed startup, persistent holds, terminal-release replacement, click completion, shutdown release, and post-acceptance physical interference.
- Both direction modes passed missing/stale motion, layout-generation restart, physical release, and expected scaling. A proxy fixture with UPT3 disabled received UPS3 followed by UPS1, received no UPS2, and recovered button-only output.

The receive/performance path was reviewed without speculative changes: reassembly retains a bounded three-frame incomplete set and seven-buffer pool, copies each fragment once, and publishes only the newest complete frame. Inference and output use latest-value slots, freshness deadlines, and event-driven handoff. The TensorRT path reuses pinned/device buffers and one CUDA stream for asynchronous upload, fused preprocessing, enqueue, and download; synchronization occurs before bounded CPU decode/NMS consumes the result. No profiling evidence from this host justified weakening freshness or adding pipeline complexity.

The production CMake path sets CUDA architecture 86 for the RTX 3060 Ti, requires a CUDA toolkit plus TensorRT 10.x headers/libraries, defines `RECEIVER_HAS_TENSORRT`, and creates `receiver_verify` only when `RECEIVER_TENSORRT=ON`. This host has an NVIDIA MX450 and driver 592.82 but no NVCC, complete TensorRT SDK, or Visual Studio toolchain, so this run proves only the non-GPU build. No TensorRT backend was linked or executed, and simulated inference timings are not GPU measurements.

Still required on the deployment hardware: a linked MSVC/CUDA/TensorRT build, `receiver_verify` and representative-image comparison, engine build/warmup/FP16 checks, RTX 3060 Ti latency and memory measurements, the proxy's Linux tests, physical Raspberry Pi/Raw Gadget/USB behavior, Logitech G Pro Wireless enumeration and interference testing, reconnect/backpressure testing, and LAN freshness/throughput measurements. None of those hardware checks is claimed as passed.
...
The Pi codec tests pass locally for unchanged UPT1, canonical 80-byte UPT2, legacy cumulative 88-byte UPT2, and cumulative UPT3 framing. The pinned proxy's incompatible 80-byte UPT2 vector is rejected, and runtime negotiation does not request UPT2. Production direction tracking uses UPT3 physical cumulative counters; no physical Pi deployment has been performed.
